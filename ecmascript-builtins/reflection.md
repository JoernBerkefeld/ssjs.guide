...
{% include test-script.html bundle="ecmascript-builtins--reflection" chapter="proxy" %}
...
Do **not** substitute the `in` operator for `Reflect.has` — it is broken in this engine. Its result has `typeof` `"undefined"` (neither `=== true` nor `=== false`), and as an `if` condition it reports an absent key on an empty object as present. See [The `in` Operator Is Unreliable](/engine-limitations/known-bugs/#in-operator-unreliable).
...
{% include test-script.html bundle="ecmascript-builtins--reflection" chapter="reflect" %}
